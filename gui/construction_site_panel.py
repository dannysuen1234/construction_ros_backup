...
gazebo_launch = 'gazebo_construction.launch'
navigation_package = 'husky_navigation '
navigation_launch = 'amcl_construction.launch'
...
def move_base(event):
	...
	move_base_demo = threading.Thread(target = linux_command, args=cmd)
...
def t1_click (event):
	cmd = 'roslaunch ' + gazebo_package + ' ' + 'construction_t1.launch'
	t1_thread = threading.Thread(target = linux_command, args=cmd)
	t1_thread.start()
...
def btn_respawn(event):
	del_robot = threading.Thread(target = linux_command, args = "rosservice call gazebo/delete_model '{model_name: husky}'")
	del_robot.start()
	spawn_robot = threading.Thread(target = linux_command, args = "roslaunch husky_gazebo spawn_husky.launch")
	spawn_robot.start()
	time.sleep(1)
	pos = threading.Thread(target = linux_command, args = position_cmd)
...
t1_btn = tk.Button(text = "Start Task 1", width =25, height =5, bg = "#618685", fg = "yellow")
t1_btn.bind("<Button-1>", t1_click)
t1_btn.pack(fill = tk.BOTH, expand = True)

respawn_btn = tk.Button(text = "respawn robot", width =25, height =5, bg = "#80ced6", fg = "yellow")
respawn_btn.bind("<Button-1>", btn_respawn)
respawn_btn.pack(fill = tk.BOTH, expand = True)

button1 = tk.Button(text = "Start Task 2 SLAM:", width =25, height =5, bg = "#618685", fg = "yellow")
...
button8 = tk.Button( text = "Start Task3 navigation", width =25, height =5, bg = "#618685", fg = "yellow")
...
frame_d = tk.Frame()
good_map = tk.Button(master = frame_d, text = "Use good map", width =15, height =5, bg = "#80ced6", fg = "yellow")
good_map.bind("<Button-1>", good_map_btn)
good_map.pack(fill=tk.BOTH, side = tk.LEFT)

bad_map = tk.Button(master = frame_d, text = "Use bad map", width =16, height =5, bg = "#80ced6", fg = "yellow")
bad_map.bind("<Button-1>", bad_map_btn)
bad_map.pack(fill=tk.BOTH, side = tk.LEFT)

pre_map = tk.Button(master = frame_d, text = "Use predefined map", width =15, height =5, bg = "#80ced6", fg = "yellow")
pre_map.bind("<Button-1>", pre_map_btn)
pre_map.pack(fill=tk.BOTH, side = tk.LEFT)
frame_d.pack()
